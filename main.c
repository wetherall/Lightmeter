/**
 * Take a light measurement with all LEDs turned off
 * 
 * This function turns off all LEDs, takes a light reading,
 * calculates the appropriate shutter speed, and then
 * restores the displays.
 * 
 * @return The calculated shutter speed in seconds
 */
float take_light_measurement(void) {
    // Store the current state of the LEDs so we can restore aperture/ISO display
    bool is_iso_mode = (display_mode == DISPLAY_ISO);
    
    // Turn off all LEDs before taking a measurement to prevent interference
    for (int i = 0; i < LEDS_PER_COLUMN; i++) {
        set_led(i, 0, false);  // Clear ISO/aperture column
        set_led(i, 1, false);  // Clear shutter speed column
    }
    
    // Brief delay to ensure LEDs are fully off
    _delay_ms(5);
    
    // Take a light reading - the VEML7700 will automatically adjust 
    // integration time based on light levels
    float lux = measure_light();
    
    // Calculate shutter speed based on light reading and current settings
    float shutter_speed = calculate_shutter_speed(lux, iso_setting, aperture_setting);
    
    // Return the calculated shutter speed
 ...
 * 7. Battery level is monitored periodically and a warning is displayed when critically low
 * 
 * Hardware Configuration:
 * - ATtiny3216 microcontroller
 * - 24 LEDs in a 6×4 matrix (direct drive)
 * - VEML7700 light sensor via I2C
 * - 4 pushbuttons for user control
 * - LIR2450 rechargeable lithium coin cell for power
 */
...
/* Battery warning display flag */
volatile bool showing_battery_warning = false;
...
volatile bool battery_blink_state = false;  // Blink state for battery warning
...
/**
 * Display a battery warning
 * 
 * This function creates a visual warning pattern when the battery
 * is critically low, alternating between all LEDs on and off.
 */
void display_battery_warning(void) {
    // Clear all LEDs
    for (int i = 0; i < LEDS_PER_COLUMN; i++) {
        set_led(i, 0, false);
        set_led(i, 1, false);
    ...
    // If in the "on" state of the blink cycle, turn on all LEDs
    if (battery_blink_state) {
        for (int i = 0; i < LEDS_PER_COLUMN; i++) {
            // Light both columns with half brightness to indicate warning
            set_led_brightness(i, 0, current_brightness / 2);
            set_led_brightness(i, 1, current_brightness / 2);
        }
    }
}
 ...
 * 
 * This function is simplified to match our two brightness levels.
 * It visually shows the current brightness setting during development.
 */
void display_brightness(void) {
    // In the simplified implementation, we've eliminated the brightness adjustment UI
    // However, we keep this function for development purposes to visualize current settings
    ...
    // Show different patterns for different brightness settings
    // This is just for development - can be modified or removed in final version
    
    // Turn on first column LEDs
    for (int i = 0; i < 6; i++) {
        set_led(i, 0, true);
    }
    
    // Turn on some second column LEDs to show we're in brightness test mode
    set_led(0, 1, true);
    set_led(5, 1, true);
}
...
ISR(TCB0_INT_vect) {
    ...
    if (current_time - last_battery_check >= BATTERY_CHECK_INTERVAL) {
        ...
        // Check battery level and set warning flag if critical
        if (is_battery_critical()) {
            showing_battery_warning = true;
        }
    }
    
    // Handle battery warning blinking
    if (showing_battery_warning) {
        static uint32_t last_battery_blink_time = 0;
        
        if (current_time - last_battery_blink_time >= BLINK_MED_MS) {
            last_battery_blink_time = current_time;
            battery_blink_state = !battery_blink_state;
            
            // Update display based on blink state
            display_battery_warning();
        }
        
        // Skip other display updates when showing battery warning
...
ISR(PORTA_PORT_vect) {
    ...
    if (PORTA.INTFLAGS & MODE_BUTTON_PIN) {
        ...
        if (!(PORTA.IN & MODE_BUTTON_PIN)) {
            ...
            if (mode_button_held) {
                ...
                if (last_lux_reading > 0) {
                    update_shutter_display(calculate_shutter_speed(
    ...
    if (PORTA.INTFLAGS & READ_BUTTON_PIN) {
        if (debounce_check(&last_read_press_time)) {
            ...
            // If we were showing a battery warning, clear it
            // (but still monitor in background)
            showing_battery_warning = false;
        }
...
int main(void) {
    ...
    // Main loop
    while (1) {
        // Check battery status at startup and periodically
        if (is_battery_critical()) {
            showing_battery_warning = true;
            display_battery_warning();
        }
        
        // Skip normal processing if displaying battery warning
        if (showing_battery_warning) {
            // Check if any button was pressed to clear the warning
            if (wake_flag || mode_button_flag || up_button_flag || down_button_flag) {
                // Clear flags
                wake_flag = mode_button_flag = up_button_flag = down_button_flag = false;
                
                // Clear the battery warning display
                showing_battery_warning = false;
            ...
            // Skip the rest of the loop while showing battery warning
            continue;
        }
        ...
        if (!wake_flag && !mode_button_flag && !up_button_flag && !down_button_flag && 
            !in_brightness_mode && !showing_battery_warning) {
